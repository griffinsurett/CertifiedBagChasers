 ...
 * Uses the i75 JS masonry layout to distribute gaps cleanly.
 */
...
const TestimonialMasonry = ({ items, className = '' }: TestimonialMasonryProps) => {
  ...
  return (
    <div
      className={`
        testimonial-masonry relative masonry-container grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4
        ${className}
      `}
    ...
    >
      {items.map((item, index) => {
        ...
        const fallbackSpanClass = item.spanColumns ? 'lg:col-span-2 tm-span-2' : '';
        const fallbackRowClass = item.spanRows ? 'lg:row-span-2 tm-row-2' : '';
